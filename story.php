...
// SEO Fallbacks
$meta_title = !empty($post['meta_title']) ? $post['meta_title'] : $post['title'] . " - " . ($settings['site_name'] ?? 'NEWS5');
...
<body class="overflow-x-hidden">
    ...
    <header class="bg-black border-b border-white/5">
        <div class="max-w-[1400px] mx-auto flex items-stretch">
            ...
            <div class="bg-red-600 px-6 flex items-center justify-center cursor-pointer">
    ...
    <main class="max-w-[1400px] mx-auto">
        <article class="space-y-12">
            ...
            <div class="relative w-full aspect-[21/9] overflow-hidden">
                ...
                <?php else: ?>
                    <div class="w-full h-full bg-zinc-900 flex items-center justify-center">
                        <span class="text-zinc-800 text-6xl font-black italic">NEWS5</span>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="max-w-3xl mx-auto px-6 pb-20">
                <header class="mb-12">
                    <h1 class="text-4xl md:text-5xl font-black leading-tight text-white mb-6 italic tracking-tight">
                        <?php echo htmlspecialchars($post['title']); ?>
                    </h1>
                    <div class="flex items-center space-x-4 text-slate-500 text-xs font-bold uppercase tracking-widest border-t border-white/5 pt-6">
                        <span class="text-red-600"><?php echo htmlspecialchars($post['category_name'] ?? 'General'); ?></span>
                        <div class="w-1 h-1 rounded-full bg-white/20"></div>
                        <span><?php echo date('M d, Y', strtotime($post['created_at'])); ?></span>
                    </div>
                </header>

                <div class="prose prose-invert max-w-none text-slate-300 leading-relaxed text-lg space-y-8 font-medium">
                    <?php echo nl2br(htmlspecialchars($post['content'])); ?>
                </div>

                <?php if ($post['youtube_url'] && ($vid = getYouTubeID($post['youtube_url']))): ?>
                    <div class="mt-16 rounded-sm overflow-hidden shadow-2xl aspect-video border border-white/5">
