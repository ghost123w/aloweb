...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM posts WHERE seo_title = ? LIMIT 1");
...
<head>
    ...
    <title><?php echo htmlspecialchars($post['title']); ?> - YourStoryline</title>
...
<body class="bg-white text-slate-900">
    ...
    <nav class="border-b border-slate-100 py-6 sticky top-0 bg-white/80 backdrop-blur-md z-50">
        <div class="max-w-4xl mx-auto px-6 flex justify-between items-center">
            <a href="index.php" class="text-2xl font-black tracking-tighter text-slate-900 uppercase">YourStoryline</a>
            <a href="index.php" class="text-sm font-bold text-slate-400 hover:text-slate-900 transition uppercase tracking-widest">Back to Stories</a>
        </div>
    </nav>

    <article class="max-w-4xl mx-auto px-6 py-20">
        <header class="text-center space-y-8 mb-16">
            <span class="text-sm font-bold text-blue-600 uppercase tracking-[0.2em]"><?php echo date('M d, Y', strtotime($post['created_at'])); ?></span>
